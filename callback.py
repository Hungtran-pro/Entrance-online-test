...
class TorchCallback:
    def on_epoch_end(self, model: torch.nn.Module, train_result: torch.Tensor, valid_result: torch.Tensor):
...
class TorchModelCheckpoint(TorchCallback):
    ...
    def on_epoch_end(self, model: torch.nn.Module, valid_result: float):
        new_loss = valid_result
        if new_loss < self.current_best_loss:
            torch.save(model.state_dict() if self.save_weights_only else model,
